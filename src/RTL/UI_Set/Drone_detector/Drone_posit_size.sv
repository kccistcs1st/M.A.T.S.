module drone_posit_size #(
    ...
    localparam int FIFO_DATA_WIDTH = 1 + 1 + $clog2(TOTAL_AREAS); // {is_eof, type, addr}
                ...
                ST_CALC: begin
                    ...
                    if (is_eof) begin
                        ...
                        if (match_found) begin
                            // 인접한 박스가 있다면 크기 확장
                            if (pop_x < box_min_x[match_idx]) box_min_x[match_idx] <= pop_x;
                            if (pop_x > box_max_x[match_idx]) box_max_x[match_idx] <= pop_x;
                            if (pop_y > box_max_y[match_idx]) box_max_y[match_idx] <= pop_y;
                ...
                ST_FLUSH: begin
                    if (flush_idx < MAX_DRONES) begin
                        if (is_active[flush_idx]) begin
                            ...
                            center_x     <= (($unsigned(box_min_x[flush_idx]) + $unsigned(box_max_x[flush_idx])) * DIV_WIDTH) / 2 + (DIV_WIDTH / 2);
                            center_y     <= (($unsigned(box_min_y[flush_idx]) + $unsigned(box_max_y[flush_idx])) * DIV_HEIGHT) / 2 + (DIV_HEIGHT / 2);
                            target_width <= ($unsigned(box_max_x[flush_idx]) - $unsigned(box_min_x[flush_idx]) + 1) * DIV_WIDTH;
                            target_height<= ($unsigned(box_max_y[flush_idx]) - $unsigned(box_min_y[flush_idx]) + 1) * DIV_HEIGHT;
